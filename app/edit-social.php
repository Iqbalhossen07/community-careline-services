...
// আইডি গেট করা এবং ডাটা ফেচ করা
if (isset($_GET['id'])) {
    $id = mysqli_real_escape_string($mysqli, $_GET['id']);
    $result = $mysqli->query("SELECT * FROM social_links WHERE id = '$id'");
...
<head>
    ...
    <title>Edit Social Link - Admin Dashboard</title>
</head>
                    ...
                    <form action="logics.php" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-6 font-body">
                            <div class="md:col-span-4">
                                ...
                                <select id="platform_name" name="platform_name" required class="w-full px-4 py-2 border rounded-lg shadow-sm focus:ring-2 focus:ring-primary-end outline-none bg-white">
                                    <?php
                                    $platforms = ['WhatsApp', 'Facebook', 'YouTube', 'TikTok', 'Instagram', 'Telegram', 'Twitter', 'LinkedIn'];
                                    foreach ($platforms as $p):
                                        $selected = ($social['platform_name'] == $p) ? 'selected' : '';
                            ...
                            <div class="md:col-span-8">
                                ...
                                <input type="url" id="link" name="link" value="<?php echo htmlspecialchars($social['link']); ?>" class="w-full px-4 py-2 border rounded-lg shadow-sm focus:ring-2 focus:ring-primary-end outline-none" required>
                            </div>
                        </div>
                        ...
                        <div class="mt-8 text-right">
                            <button type="submit" name="update_social" class="font-nav inline-flex items-center justify-center bg-primary-start text-white px-8 py-3 rounded-lg shadow-md hover:bg-primary-end transition-colors duration-300 font-bold uppercase tracking-widest text-sm">
                                <svg class="w-5 h-5 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
