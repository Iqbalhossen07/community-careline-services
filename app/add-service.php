                ...
                <div class="bg-white rounded-xl shadow-lg p-6 md:p-8" data-aos="fade-up">
                    <form action="logics.php" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="bg-gray-50 rounded border border-gray-200 p-6 mb-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="mb-4">
                                    ...
                                    <input type="text" name="title" class="font-body w-full px-4 py-2 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-end focus:border-transparent" required placeholder="Type service name">
                                </div>
                           

                                <div>
                                    <label class="block text-sm font-semibold text-gray-700 mb-1">Category Type</label>
                                    <select name="category" class="w-full px-4 py-2 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-end focus:border-transparent bg-white font-body" required>
                                        <option value="" disabled selected>Select Category</option>

                                        <?php
                                        // ডাটাবেস থেকে ক্যাটাগরিগুলো ফেচ করা
                                        $cat_query = $mysqli->query("SELECT * FROM categories ORDER BY name ASC");

                                        if ($cat_query && $cat_query->num_rows > 0) {
                                            while ($row = $cat_query->fetch_assoc()) {
                                                // ডাটাবেসের নাম অনুযায়ী অপশন তৈরি করা
                            ...
                            <div>
                                ...
                                <textarea id="textarea-description" placeholder="Type description" name="description" rows="4" class="font-body w-full px-3 py-2 border rounded"></textarea>
                            </div>
                        </div>

                        <div class="bg-gray-50 rounded border border-gray-200 p-6 mb-6">
                            <label class="font-body block text-sm font-semibold text-gray-700 mb-2">Service Images</label>
                            <label for="file-upload" class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-primary-end border-dashed rounded-md cursor-pointer hover:bg-gray-100 transition-colors bg-white">
                                <div class="space-y-1 text-center w-full">

                                    <div id="preview-container" class="hidden grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                                    </div>
                        ...
                        <div class="mt-6 text-right">
                            <button type="submit" name="add_service" class="font-nav bg-primary-start text-white px-6 py-2 rounded shadow hover:bg-primary-end transition font-bold uppercase tracking-wider">Save Service</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            if (files.length > 0) {
                ...
                Array.from(files).forEach(file => {
                    ...
                    reader.onload = function(e) {
                        ...
                        imgWrapper.className = 'relative w-full h-32 rounded-lg overflow-hidden shadow-sm border border-gray-200';
